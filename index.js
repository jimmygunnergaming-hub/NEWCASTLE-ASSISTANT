...
// -------------------------------------------------------------
// DYNAMIC WEB STADIUM INTERFACE (PITCH DASHBOARD)
// -------------------------------------------------------------
  ...
  res.send(`
    ...
    <head>
      ...
      <style>
        ...
        .center-circle { position: absolute; top: 50%; left: 50%; width: 140px; height: 140px; border: 4px solid #fff; border-radius: 50%; transform: translate(-50%, -50%); }
        .penalty-box-top { position: absolute; top: 0; left: 50%; width: 260px; height: 120px; border: 4px solid #fff; transform: translateX(-50%); border-top: none; }
        .penalty-box-bot { position: absolute; bottom: 0; left: 50%; width: 260px; height: 120px; border: 4px solid #fff; transform: translateX(-50%); border-bottom: none; }
        
        .player-node {
          position: absolute; width: 70px; height: 70px; background: #2c3e50;
        ...
        .label-container { position: absolute; bottom: -45px; width: 120px; font-size: 11px; font-weight: bold; background: rgba(0,0,0,0.85); border-radius: 4px; padding: 2px; border: 1px solid #444; }
        ...
        .member-card:hover { background: #f1c40f; color: #000; }
      </style>
    </head>
    <body class="container-fluid py-3">
      <h3>⚽ NEWCASTLE ASSISTANT STRATEGY CONSOLE</h3>
      <p class="text-muted">Drag circles anywhere. Click a circle to name its position and assign players!</p>
          ...
          return `
            <div class="player-node" id="node_${idx}" style="left: ${pctX}%; top: ${pctY}%;" 
              ...
              <div id="avatar_box_${idx}" class="w-100 h-100 d-flex align-items-center justify-content-center">
                <span style="font-size:20px; color:#95a5a6;">⚪</span>
              </div>
      ...
      <script>
        ...
        function saveAndPostToDiscord() {
          ...
          }).then(() => alert('Lineup finalized! You can close this tab and check Discord.'));
        }
      </script>
    </body>
    </html>
