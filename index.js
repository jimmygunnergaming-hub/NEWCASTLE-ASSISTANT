...
client.on('interactionCreate', async (interaction) => {
  ...
  // 2. PLAYER REPETITIVE SELECTION DROPDOWN HANDLER
...
async function generatePitch(interaction, msgId, isEdit) {
  ...
  const coords = [{ x: width / 2, y: height - 100 }]; // GK is always fixed at base
...
async function finishLineup(interaction, msgId) {
  ...
  } catch (err) { console.error('Discord routing delivery channel fetch error:', err); }
